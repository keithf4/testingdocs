 ...
 * Had to disable opening linked dashboards in a new tab. Bug in Grafana is causing imported dashboards that are linked to be broken when using new tab option. See above issue with 4.x to 5.x upgrade.
 * Fixed Memory and Swap Graphs on PostgreSQLDetails dashboard to more accurately show used resources. History for these graphs before this upgrade is unable to shown since it is no longer graphing the same data.
 ...
 * Change PostgreSQL overview to use background colors instead of gauges for better visibility.
