...
delete from partman.part_config where parent_table = 'tracking.hits';
create schema if not exists tracking;
create table tracking.hits(id int not null, start timestamp not null );
create table tracking.hits20160103 (like tracking.hits) inherits (tracking.hits);
create table tracking.hits20160110 (like tracking.hits) inherits (tracking.hits);
create table tracking.hits20160117 (like tracking.hits) inherits (tracking.hits);
...
create schema if not exists tracking;
create table tracking.hits(id int not null, start timestamp not null );
create table tracking.hits1000 (like tracking.hits) inherits (tracking.hits);
create table tracking.hits2000 (like tracking.hits) inherits (tracking.hits);
create table tracking.hits3000 (like tracking.hits) inherits (tracking.hits);

insert into tracking.hits1000 values (generate_series(1000,1999), now());
insert into tracking.hits1000 values (generate_series(2000,2999), now());
insert into tracking.hits1000 values (generate_series(3000,3999), now());

drop schema if exists tracking cascade;
delete from partman.part_config where parent_table = 'tracking.hits';
create schema if not exists tracking;
create table tracking.hits(id int not null, start timestamp not null );
create table tracking.hits_aa (like tracking.hits) inherits (tracking.hits);
create table tracking.hits_bb (like tracking.hits) inherits (tracking.hits);
create table tracking.hits_cc (like tracking.hits) inherits (tracking.hits);

Make a note somewhere that moving to a smaller interval is possible without repartitioning everthing. Just rename the larger partitions with the smaller interval's naming pattern and things should just work
Ex. Change monthly _pYYYY_MM to daily _pYYYY_MM_DD and set the "day" value in the suffix to the lowest day contained in that child table.

END TODO REMOVE

TODO Add some notes about how to move to a smaller partition interval 
    - Should just be able to change the interval in part_config. test this. Only need to do additional work if you actually want to change the suffix as well.
